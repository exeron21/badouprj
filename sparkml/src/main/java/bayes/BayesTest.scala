...
object BayesTest {
  def main(args: Array[String]): Unit = {
    ...
    val modelPath = "hdfs://master:9000/bayes"
    val conf = new SparkConf().registerKryoClasses(Array(classOf[JiebaSegmenter])).set("spark.rpc.message.maxSize", "800")

    val ss = SparkSession.builder().config(conf).master("local[*]").appName("BayesTest").enableHiveSupport().getOrCreate()
    ...
    val pipeLine = new Pipeline().setStages(Array(tf, idf, stringIndexer,nb))
    ...
    // 0-auto 1-it 2-sports 3-business 4-
    val f1Score = eval.evaluate(model)
    println("Test f1 score = " + f1Score)
    model.write.mode(SaveMode.Append).save(modelPath)
  }
}
